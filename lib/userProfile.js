...
export function calculateProfile(existingProfile, sessionMetrics) {
  ...
  // NEW: Increased penalty for retries to be more sensitive to user struggle
  const retryPenalty = Math.min(retryCount / Math.max(messageCount, 1), 1) * 0.6;
  const lengthBonus = Math.min(avgMessageLength / 250, 1) * 0.4;
  const sessionConfidence = Math.max(0, Math.min(1, 0.4 + lengthBonus - retryPenalty));

  // ---- Patience heuristic ----
  // NEW: More granular time-based patience detection
  // Slow responses (> 15s) → patient; very fast (< 3s) → impatient
  const normalizedTime = Math.min(avgResponseTimeMs / 20000, 1);
  const sessionPatience = normalizedTime;

  // ---- Depth preference ----
  let depthPreference = 'intermediate';
  if (avgMessageLength > 180 || base.patience > 0.75) depthPreference = 'deep';
  if (avgMessageLength < 30 || base.patience < 0.25) depthPreference = 'surface';

  // ---- Learning style (can be refined via content analysis in the future) ----
  let learningStyle = base.learningStyle || 'practical';

  // ---- Engagement ----
  const engagementScore = Math.min(
    (messageCount / 15) * 0.6 + base.engagementScore * 0.4,
    1
  );

  // Weighted blend: new session counts for 35%, history for 65% (faster adaptation)
  const blend = (oldVal, newVal, weight = 0.35) =>
  ...
  return {
    ...
    retryRate: blend(base.retryRate || 0, retryPenalty / 0.6), // normalized
  };
}
