      ...
      <div className={`max-w-[85%] md:max-w-[70%] ${isUser ? 'items-end' : 'items-start'} flex flex-col gap-1`}>
        <div
          className={`px-4 py-3 rounded-3xl text-sm leading-relaxed shadow-sm ${
            isUser
              ? 'bg-brand-500 text-white rounded-tr-sm border-brand-600'
              : 'bg-surface-50 text-surface-900 rounded-tl-sm border border-surface-200'
          }`}
        >
          {isUser ? (
            <p className="whitespace-pre-wrap break-words">{message.content}</p>
          ) : (
            <div className="prose prose-sm max-w-none prose-headings:text-surface-900 prose-p:text-surface-800 prose-code:bg-surface-100 prose-code:rounded prose-code:px-1 prose-pre:bg-surface-900 prose-pre:text-green-300 prose-strong:text-surface-900 prose-a:text-brand-600">
